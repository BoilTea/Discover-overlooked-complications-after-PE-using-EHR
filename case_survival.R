...
diag_date = diag_date %>%
    ...
    mutate(start_time = min(DeID_AdmitDate[grepl("O14", TermCodeMapped)])) %>%
...
case_survival_list = list()

# Combine data
for (disease in target) {
    case_survival_list[[disease]] = diag_date_list[[disease]] %>% 
        left_join(como_earliest, by = "DeID_PatientID") %>%
        left_join(social_hist, by = "DeID_PatientID") %>%
        left_join(race_hist, by = "DeID_PatientID")
}

save(case_survival_list, file = "case_survival_list.RData")
