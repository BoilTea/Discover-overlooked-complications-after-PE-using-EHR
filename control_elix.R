...
for (i in 1:10) {
    diag_date_i = diag_date %>%
        ...
        filter(max(DeID_AdmitDate) >= (min(DeID_AdmitDate[grepl("Z34", TermCodeMapped)]) + years(i))) %>%
        filter(DeID_AdmitDate < (min(DeID_AdmitDate[grepl("Z34", TermCodeMapped)]) + years(i)))
...
save(control_diag_all_elix_list, file = "control_diag_all_elix_list.RData")
...
save(control_stats_elix_list, file = "control_stats_elix_list.RData")
